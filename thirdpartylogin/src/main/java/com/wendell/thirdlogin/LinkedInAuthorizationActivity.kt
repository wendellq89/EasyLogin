...
class LinkedInAuthorizationActivity : AppCompatActivity() {
    ...
    private fun getRequestToken() {
        ...
        val authUrl =
            Constants.LINKEDIN_AUTHURL + "?response_type=code&client_id=" + getString(R.string.linkedin_client_id) + "&scope=" + Constants.LINKEDIN_SCOPE + "&state=" + state + "&redirect_uri=" + getString(
    ...
    private fun linkedInRequestForAccessToken(linkedinCode: String) {
        GlobalScope.launch(Dispatchers.Default) {
            ...
            val url = URL(Constants.LINKEDIN_TOKENURL)
    ...
    private fun requestUserEmail(accessToken: String): String {
        val url =
            URL("${Constants.LINKEDIN_EMAIL}?q=members&projection=(elements*(handle~))&oauth2_access_token=$accessToken")
        ...
        Log.d("email:", JSON.parseObject(response).toJSONString())
        return getUserEmail(JSON.parseObject(response))

    }

    private fun requestUserInfo(accessToken: String): AuthorizationInfo {
        val url =
            URL("${Constants.LINKEDIN_ME}?projection=(id,firstName,lastName,profilePicture(displayImage~:playableStreams))&oauth2_access_token=$accessToken")
        val httpsURLConnection = url.openConnection() as HttpsURLConnection
        httpsURLConnection.apply {
            requestMethod = "GET"
            doInput = true
        ...
        val userInfoObject = JSON.parseObject(response)
    ...
    private fun getUserBaseInfo(userInfoObject: JSONObject): AuthorizationInfo {
        ...
        val firstName = if (userInfoObject.containsKey("localizedFirstName")) {
            userInfoObject.getString("localizedFirstName")
        } else {
            getUserName(userInfoObject, "firstName")
        }
        val lastName = if (userInfoObject.containsKey("localizedLastName")) {
    ...
    private fun getUserId(userInfoObject: JSONObject): String {
        ...
        return if (userInfoObject.containsKey("id")) {
